...
if (request()->routeIs('index.shop-recalls')) {
    $rout_recall = route('index.shop-recalls', ['shop' => $current_slug]);
    $rout_all = route('shops.reviews', ['shop' => $current_slug]);
} else {
    $rout_recall = route('index.recalls');
...
<div class="recall__box">
    <div class="recall__header">
        ...
        <select class="recall__header-select">
            <option value="{{ route('index.recalls') }}">Все сексшопы</option>
            @foreach ($shops as $shop)
                <option value="{{ route('index.shop-recalls', $shop) }}" {{ $current_slug === $shop->slug ? 'selected': '' }}>
                    {{ $shop->name }}
                </option>
            @endforeach
        </select>

...
class CrutchImage
{
     ...
     */
    protected array|false|null $exif = null;
    ...
    public function __construct(?string $path = null)
    {
        ...
        if (null !== $path) {
            if (preg_match('/^data:(.*?);/', $path, $matches)) {
                $this->fromDataUri($path, $matches[1]);
            } else {
                $this->fromFile($path);
            }
        }
     ...
     * @return static
     */
    public function fromDataUri(string $uri, string $mimeType): static
    {
        if (!preg_match('/^image\/(gif|jpeg|png)$/', $mimeType)) {
            throw new InvalidArgumentException("Unsupported format: {$mimeType}", static::ERR_UNSUPPORTED_FORMAT);
        }
    ...
    public function fromFile(string $file): static
    {
        if (false === $handle = @fopen($file, 'rb')) {
            throw new DomainException("File not found: {$file}", static::ERR_FILE_NOT_FOUND);
        }

        fclose($handle);

        if (false === $info = @getimagesize($file)) {
            throw new InvalidArgumentException("Invalid image file: $file", static::ERR_INVALID_IMAGE);
        }
        ...
        switch ($info['mime']) {
            ...
            case 'image/gif':
            {
                ...
                $width = (int)imagesx($gif);
                $height = (int)imagesy($gif);
            ...
            default:
            {
                throw new InvalidArgumentException(
                    "Unsupported format: {$info['mime']}",
                    static::ERR_UNSUPPORTED_FORMAT
                );
            }
        }
     ...
     */
    protected function generate(?string $mimeType = null, int $quality = 100): array
    {
        // Format defaults to the original mime type
        $mimeType = $mimeType ?: $this->mimeType;

        $quality = (int)static::keepWithin($quality, 0, 100);
     ...
     */
    public function toDataUri(?string $mimeType = null, int $quality = 100): string
    {
     ...
     * @return static
     */
    public function toDownload(string $filename, ?string $mimeType = null, int $quality = 100): static
    {
        $image = $this->generate($mimeType, $quality);
    ...
    public function toFile(string $file, ?string $mimeType = null, int $quality = 100): static
    {
        ...
        if (!file_put_contents($file, $image['data'])) {
            throw new DomainException("Failed to write image to file: $file", static::ERR_WRITE);
        }
     ...
     * @return static
     */
    public function toScreen(?string $mimeType = null, int $quality = 100): static
    {
        $image = $this->generate($mimeType, $quality);
     ...
     * @return string
     */
    public function toString(?string $mimeType = null, int $quality = 100): string
    {
     ...
     *
     * @return int|float Returns the aspect ratio as a float.
     */
    public function getAspectRatio(): int|float
    {
     ...
     *
     * @return array|null|false Returns an array of exif data or null if no data is available.
     */
     ...
     *
     * @return array|bool The resolution as an array of int: [96, 96]
     */
    public function getResolution(): array|bool
    {
     ...
     *
     * @param GdImage $dstIm Destination image link resource.
     * @param GdImage $srcIm Source image link resource.
    ...
    protected static function imageCopyMergeAlpha(
        ...
        if ($pct < 100) {
            ...
            imagefilter($srcIm, IMG_FILTER_COLORIZE, 0, 0, 0, (int)(127 * ((100 - $pct) / 100)));
        }
    ...
    public function crop(int|float $x1, int|float $y1, int|float $x2, int|float $y2): static
    {
        ...
        $x1 = static::keepWithin($x1, 0, $this->getWidth());
        $x2 = static::keepWithin($x2, 0, $this->getWidth());
        $y1 = static::keepWithin($y1, 0, $this->getHeight());
        $y2 = static::keepWithin($y2, 0, $this->getHeight());
    ...
    {
        $lightColor = static::normalizeColor($lightColor);
        $darkColor = static::normalizeColor($darkColor);
     ...
     */
    public function fitToHeight(int $height): static
    {
     ...
     *
     * @param static|string $overlay                 The image to overlay. This can be a filename, a data URI, or a SimpleImage object.
    ...
    public function overlay(
        ...
        // Convert opacity
        $opacity = static::keepWithin($opacity, 0, 1) * 100;
     ...
     *
     * @param int|float|null $width  The new image width.
     * @param int|float|null $height The new image height.
     *
     * @return static
     */
    public function resize(int|float|null $width = null, int|float|null $height = null): static
    {
    ...
    public function rotate(int $angle, string|array $backgroundColor = 'transparent'): static
    {
        $this->image = imagerotate(
            $this->image,
            -((float)static::keepWithin($angle, -360, 360)),
    ...
    public function text(string $text, array $options, ?array &$boundary = null): static
    {
        ...
        if (!function_exists('imagettftext')) {
            throw new DomainException(
                ...
                static::ERR_FREETYPE_NOT_ENABLED
            );
        }
        ...
        $fontFile = (string)$options['fontFile'];
        ...
        if (!$boxText = imagettfbbox((float)$size, (float)$angle, $fontFile, $text)) {
            throw new DomainException("Unable to load font file: $fontFile", static::ERR_FONT_FILE);
        }
        ...
        if (true === $baselineAlign) {
            ...
            $boxFull = imagettfbbox((float)$size, (float)$angle, $fontFile,
        ...
        // Draw the text
        imagettftext($this->image, (float)$size, (float)$angle, $x, $y, $color, $fontFile, $text);
     ...
     *                     - color (string|array) - The text color (default black).
     *                     - anchor (string) - The anchor point: 'center', 'top', 'bottom', 'left', 'right', 'top left', 'top right', 'bottom left', 'bottom right' (default 'center').
     *                     - xOffset (int) - The horizontal offset in pixels (default 0). Has no effect when anchor is 'center'.
     *                     - yOffset (int) - The vertical offset in pixels (default 0). Has no effect when anchor is 'center'.
     *                     - shadow (array) - Text shadow params.
     *                     - x* (int) - Horizontal offset in pixels.
     *                     - y* (int) - Vertical offset in pixels.
     *                     - color* (string|array) - The text shadow color.
    ...
    public function textBox(string $text, array $options = []): static
    {
        ...
        $calculateOffsetFromEdge = $options['calculateOffsetFromEdge'];
        $maxWidth = (int)$options['width'];
        $leading = $options['leading'];
        $leading = static::keepWithin($leading, ($fontSizePx * -1), $leading);
        ...
        [$lines, $isLastLine, $lastLineHeight] = $this->textSeparateLines($text, $fontFile, (float)$fontSize,
            $maxWidth);

        $maxHeight = (int)((count($lines) - 1) * ($fontSizePx * 1.2 + $leading) + $lastLineHeight);

        $imageText = new static();
        ...
        if ($align !== 'justify') {
            ...
            foreach ($lines as $keyLine => $line) {
                ...
                foreach ($words as $key => $word) {
                    $wordBox = imagettfbbox((float)$fontSize, 0, $fontFile, $word);
    ...
    private function textSeparateLines(string $text, string $fontFile, float $fontSize, int $maxWidth): array
    {
        ...
        $boxFull = imagettfbbox($fontSize, 0, $fontFile,
            'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890');
     ...
     * Draws an arc.
     *
     * @param int|float    $x         The x coordinate of the arc's center.
     * @param int|float    $y         The y coordinate of the arc's center.
     * @param int|float    $width     The width of the arc.
     * @param int|float    $height    The height of the arc.
     * @param int|float    $start     The start of the arc in degrees.
     * @param int|float    $end       The end of the arc in degrees.
    ...
    public function arc(
        ...
        // Draw an arc
        if ($thickness === 'filled') {
            imagefilledarc($this->image, (int)$x, (int)$y, (int)$width, (int)$height, (int)$start, (int)$end,
                $tempColor, IMG_ARC_PIE);
        } elseif ($thickness === 1) {
            imagearc($this->image, (int)$x, (int)$y, (int)$width, (int)$height, (int)$start, (int)$end, $tempColor);
        } else {
            // New temp image
            $tempImage = new static();
            ...
            imagefilledarc($tempImage->image, (int)$x, (int)$y, $width + $thickness, $height + $thickness, (int)$start,
                (int)$end, $tempColor, IMG_ARC_PIE);

            // Draw a smaller ellipse filled with red|blue (-$thickness pixels)
            $tempColor = static::normalizeColor($color)['red'] === 255 ? 'blue' : 'red';
    ...
    public function border(string|array $color, int $thickness = 1): static
    {
        ...
        imagerectangle($this->image, $x1, $y1, $x2, $y2, $this->allocateColor($color));
    ...
    {
        imagesetpixel($this->image, $x, $y, $this->allocateColor($color));
    ...
    public function fill(string|array $color): static
    {
        ...
        $new_color = $this->allocateColor($color);
        imagefill($this->image, 0, 0, $new_color);
    ...
    public function line(int $x1, int $y1, int $x2, int $y2, string|array $color, int $thickness = 1): static
    {
        ...
        imageline($this->image, $x1, $y1, $x2, $y2, $this->allocateColor($color));
     ...
     * @return static
     */
    public function polygon(array $vertices, string|array $color, int|string $thickness = 1): static
    {
        // Allocate the color
        $allocate_color = $this->allocateColor($color);

        // Convert [['x' => x1, 'y' => x1], ['x' => x1, 'y' => y2], ...] to [x1, y1, x2, y2, ...]
        $points = [];

        foreach ($vertices as $val) {
            $points[] = $val['x'];
            $points[] = $val['y'];
        }
        ...
        if ($thickness === 'filled') {
            imagesetthickness($this->image, 1);
            imagefilledpolygon($this->image, $points, count($vertices), $allocate_color);
        } else {
            imagesetthickness($this->image, $thickness);
            imagepolygon($this->image, $points, count($vertices), $allocate_color);
        }
     ...
     * Draws a rectangle.
     *
     * @param int|float    $x1        The upper left x coordinate.
     * @param int|float    $y1        The upper left y coordinate.
     * @param int|float    $x2        The bottom right x coordinate.
     * @param int|float    $y2        The bottom right y coordinate.
    ...
    public function rectangle(
        ...
        // Allocate the color
        $r_color = $this->allocateColor($color);

        // Draw a rectangle
        if ($thickness === 'filled') {
            imagesetthickness($this->image, 1);
            imagefilledrectangle($this->image, (int)$x1, (int)$y1, (int)$x2, (int)$y2, $r_color);
        } else {
            imagesetthickness($this->image, $thickness);
            imagerectangle($this->image, (int)$x1, (int)$y1, (int)$x2, (int)$y2, $r_color);
        }
     ...
     * @param int|float    $x2        The bottom right x coordinate.
     * @param int|float    $y2        The bottom right y coordinate.
     * @param int|float    $radius    The border radius in pixels.
     * @param string|array $color     The rectangle color.
     * @param int|string   $thickness Line thickness in pixels or 'filled' (default 1).
     *
     * @return static
    ...
    public function roundedRectangle(
        ...
        if ($thickness === 'filled') {
            ...
            $y1 -= $offset;
            $y2 += $offset;
            $radius = static::keepWithin($radius, 0, min(($x2 - $x1) / 2, ($y2 - $y1) / 2) - 1);
            $radius = floor($radius);
            $new_thickness = static::keepWithin($thickness, 1, min(($x2 - $x1) / 2, ($y2 - $y1) / 2));
            ...
            $tempColor = static::normalizeColor($color)['red'] === 255 ? 'blue' : 'red';
            $radius -= $new_thickness;
            $radius = static::keepWithin($radius, 0, $radius);
     ...
     *
     * @param int|float    $x           certer x of rectangle.
     * @param int|float    $y           certer y of rectangle.
     * @param string|array $borderColor The color of border.
     *
     */
    protected function excludeInsideColor(int|float $x, int|float $y, string|array $borderColor): void
    {
        $border_color = $this->allocateColor($borderColor);
        $transparent = $this->allocateColor('transparent');
        imagefilltoborder($this->image, (int)$x, (int)$y, $border_color, $transparent);
    }
    ...
    {
        imagefilter($this->image, IMG_FILTER_BRIGHTNESS, (int)static::keepWithin(255 * $percentage / 100, 0, 255));
    ...
    {
        $color = static::normalizeColor($color);
    ...
    {
        imagefilter($this->image, IMG_FILTER_CONTRAST, (int)static::keepWithin($percentage, -100, 100));
    ...
    {
        $percent = (int)static::keepWithin(255 * $percentage / 100, 0, 255);
     ...
     *
     * @param int|float $opacity The desired opacity level (0 - 1).
     *
     * @return static
     */
    public function opacity(int|float $opacity): static
    {
        ...
        static::imageCopyMergeAlpha(
            ...
            $this->getWidth(),
            $this->getHeight(),
            static::keepWithin($opacity, 0, 1) * 100
        );
    ...
    {
        // Normalize amount
        $amount_n = max(1, min(100, $amount)) / 100;
     ...
     * @param int|float    $alpha Alpha adjustment (-1 - 1).
     *
     * @return int[] An RGBA color array.
     */
    public static function adjustColor(string|array $color, int $red, int $green, int $blue, int|float $alpha): array
    {
        // Normalize to RGBA
        $color = static::normalizeColor($color);

        // Adjust each channel
        return static::normalizeColor([
            'red'   => $color['red'] + $red,
    ...
    {
        return static::adjustColor($color, -$amount, -$amount, -$amount, 0);
    }
     ...
     *
     * @return array An RGBA color array or false if the x/y position is off the canvas.
     */
    public function getColorAt(int $x, int $y): array
    {
        // Coordinates must be on the canvas
        if ($x < 0 || $y < 0 || $x > $this->getWidth() || $y > $this->getHeight()) {
            throw new InvalidArgumentException('Coordinates must be on the canvas');
        }
     ...
     * @throws InvalidArgumentException Thrown if color value is invalid.
     */
    public static function normalizeColor(string|array $color): array
    {
        ...
        if (is_string($color) && str_contains($color, '|')) {
            $color = explode('|', $color);
            $alpha = (float)$color[1];
            $color = trim($color[0]);
        } else {
        ...
        if (is_string($color)) {
            ...
            if (strlen($hex) === 3) {
                ...
                throw new InvalidArgumentException("Invalid color value: $color", static::ERR_INVALID_COLOR);
            }
        ...
        if (is_array($color)) {
            ...
            return [
                'red'   => (int)static::keepWithin((int)$color['red'], 0, 255),
                'green' => (int)static::keepWithin((int)$color['green'], 0, 255),
                'blue'  => (int)static::keepWithin((int)$color['blue'], 0, 255),
                'alpha' => static::keepWithin($color['alpha'], 0, 1)
            ];
        }
